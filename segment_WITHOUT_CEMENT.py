...
if InputDialog.AskYesNoQuestion('Align', 'Have you aligned the model?') == True:
	...
	#Find Loading Point
	firstYmin = firstXmin = 200
	doc.GetMaskByName("Superior_endcap").Activate()
	ymin = xmin = 0
	voxelNumBound=120

	for x in range (15, doc.GetSliceCount(Doc.OrientationYZ)-15):
		for y in range (15, doc.GetSliceCount(Doc.OrientationZX)-15):
			curYVal=doc.GetMaskByName("Superior_endcap").GetVoxelValue(x,y,doc.GetSliceCount(Doc.OrientationXY)-2)
			if curYVal > voxelNumBound:
				if y<ymin:
					ymin=y
					if ymin < firstYmin :
						firstYmin =ymin
						#App.GetInstance().ShowMessage(str(firstYmin))
					break
				ymin = y
	for y in range (15, doc.GetSliceCount(Doc.OrientationZX)-15):
		for x in range (15, doc.GetSliceCount(Doc.OrientationYZ)-15):
			...
			if curYVal > voxelNumBound:
				if x<xmin:
					...
					break
				xmin = x
	App.GetInstance().ShowMessage('Centre point is: ' + str(firstXmin+5) + ',' + str(firstYmin+5) +',' + str(doc.GetSliceCount(Doc.OrientationXY)))
	...
	doc.GetMaskByName("Vertebrae").Activate()
	...
	doc.GetMaskByName("Superior_endcap").Activate()
	doc.FloodFillFromActiveMask(35, 15, 54, False, Doc.Mode3D, Doc.ReplaceWithMask, doc.GetSliceIndices(Doc.OrientationYZ), Doc.OrientationYZ)
	doc.GetMaskByName("Inferior_endcap").Activate()
	doc.FloodFillFromActiveMask(35, 15, 1, False, Doc.Mode3D, Doc.ReplaceWithMask, doc.GetSliceIndices(Doc.OrientationYZ), Doc.OrientationYZ)
